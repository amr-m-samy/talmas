...
function Abacus(parentDivId, type, scale) {
  ...
  this.update = function () {
    ...
    ctx.font = "20pt serif";
    // ctx.color = "white";
    var textY = abacusCtrl.beadPerLine + 27 * abacusCtrl.scale;
    for (var i = 0; i < abacusCtrl.beadLines; i++) {
      if (i === 2) continue;
      var textX =
        -(30 * abacusCtrl.scale) +
    ...
    if (!abacusCtrl.nodes[dragging].active) {
      ...
      for (var i = 0; i < draggingBeads; i++) {
        if (!abacusCtrl.nodes[dragging + i].active) {
          if (event.offsetY) beadMouseY = event.offsetY - halfBeadHeight;
          else beadMouseY = event.targetTouches[0].clientY - 150;
