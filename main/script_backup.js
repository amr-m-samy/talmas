...
function AbacusCtrl(type) {
  ...
  this.init = function () {
    ...
    for (var i = 0; i < this.beadLines; i++) {
      ...
      for (var j = 0; j < this.beadPerLine; j++) {
        var bead = new Bead();
        bead.position[0] = 340 - i * this.beadSpacing; //my changes
        bead.position[1] =
  ...
  this.activated = function (nodeId) {
    ...
    let plus_offset;
    plus_offset =
      !(nodeId === 15 && this.nodes[nodeId].position[1] === 260) && // this condition ignore first auto activated   abacusCtrl.activated(15);
      !(nodeId === 5 && this.nodes[nodeId].position[1] === 260) && // this condition ignore first auto activated   abacusCtrl.activated(5);
      nodeId > 0 &&
      nodeId < 20
        ? (plus_offset = 6)
        : (plus_offset = 1);
...
function Abacus(parentDivId, type) {
  ...
  this.init = function () {
    ...
    canvas.width = 40 + abacusCtrl.beadLines * abacusCtrl.beadSpacing * scale;
    canvas.height =
      60 + (abacusCtrl.beadPerLine + 2) * abacusCtrl.beadHeight * scale;
  ...
  this.update = function () {
    ...
    var textY = abacusCtrl.beadPerLine + 10;
    for (var i = 0; i < abacusCtrl.beadLines; i++) {
      var textX =
        -30 +
        abacusCtrl.beadLines * abacusCtrl.beadSpacing -
  ...
  function draggingHandler(event) {
    ...
    const startY = 60;
    const endY = 460;
    const halfBeadHeight = beadHeight / 2;
    let beadMouseY;
    let beadPos;
    let maxBeadY;
    let minBeadY;
    ...
    abacusCtrl.nodes[dragging].position[1] = event.offsetY - 20;
    ...
    if (!abacusCtrl.nodes[dragging].active) {
      ...
      for (var i = 0; i < draggingBeads; i++) {
        if (!abacusCtrl.nodes[dragging + i].active) {
          beadMouseY = event.offsetY
            ? event.offsetY - halfBeadHeight
            : event.targetTouches[0].clientY - 150;
      ...
      for (var i = 0; i < draggingBeads; i++) {
        if (abacusCtrl.nodes[dragging - i].active) {
          beadMouseY = event.targetTouches[0].clientY;
          beadMouseY = event.offsetY
            ? event.offsetY - halfBeadHeight
            : event.targetTouches[0].clientY - 150;
          // beadMouseY = event.offsetY - halfBeadHeight;
          beadPos = beadMouseY + i * beadHeight;
  ...
  function canvasMouseDown(event) {
    ...
    if (!event.altKey && event.which === 1) {
      ...
      window.addEventListener("mouseup", function (windowMouseUpEvent) {
        ...
        if (windowMouseUpEvent.originalTarget.tagName !== "CANVAS") {
          canvasMouseUp(event);
        }
  ...
  function canvasMouseUp(event) {
    ...
    // handle selection
    restorOriginalY();
    // abacusCtrl.nodes[dragging].position[1] = originalY;
  ...
  function drawRoundRectFilled(ctx, x, y, width, height, radius) {
    ...
    // ctx.lineJoin = "round";
    // ctx.lineWidth = radius; // Fill upper half with lighter green
    ctx.beginPath();
    ...
    if (x === 100) {
      ...
      gradientTop.addColorStop(0, "#96E9C6");
